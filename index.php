...
class Shop {
    ...
    boolean bookIsAvailable(String title){
        ...
        for(int i = 0;i<size;i++){
            if(bookList.get(i).bookTitle.equals(title)){
                avail = true;
                break;
            }
        }
    ...
    List<String> bookListByAuthor(String writer){
        List<String> bookAuthor = new ArrayList<String>();
        int size  = bookList.size();
        for(int i = 0;i<size;i++){
            if(bookList.get(i).bookWriter.equals(writer)){
                bookAuthor.add(bookList.get(i).bookTitle);
            }
        }
        return bookAuthor;
    }

    List<String> bookListByTitleContains(String title){
        List<String> bookTitles = new ArrayList<String>();
        int size  = bookList.size();
        title = title.toLowerCase();
        for(int i = 0;i<size;i++){
            if(bookList.get(i).bookTitle.toLowerCase().contains(title)){
                bookTitles.add(bookList.get(i).bookTitle);
            }
        }
        return bookTitles;
    }
...
class Book {
    ...
    // PLACE YOUR CODE HERE
    String bookTitle, bookWriter;

    Book(String bT, String bW){
        bookTitle = bT;
        bookWriter = bW;
    }
}
...
class Person {
    ...
    // PLACE YOUR CODE HERE
    String name;
    List<Book> bookPerson;

    Person(String nm){
        name = nm;
        bookPerson = new ArrayList<Book>();
    }
    ...
    void removeFromBag(String title){
        int size  = bookPerson.size();
        for(int i = 0;i<size;i++){
            if(bookPerson.get(i).bookTitle.equals(title)){
                bookPerson.remove(i);
                break;
            }
        }
